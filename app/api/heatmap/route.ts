...
function renderRadialSVG(commits: any[], weekStart: Date, weekEnd: Date, username: string, theme = "green") {
  ...
  commits.forEach((c: any) => {
    ...
    heatmap[day][hour] = c.contributionCount;
  });

  return `
<svg width="${width}" height="${width + 40}" viewBox="0 0 ${width} ${width + 40}" xmlns="http://www.w3.org/2000/svg" style="background:${background}; font-family:sans-serif;">
  <!-- Concentric Rings -->
  ...
  <!-- Caption -->
  <text x="${center}" y="${width + 20}" fill="#94a3b8" font-size="14" text-anchor="middle">
